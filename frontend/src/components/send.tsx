...
export function Send() {
  ...
  const [polygonAddr, setpolygonAddr] = useState<string>(
  ...
  const [incogniaIDError, setIncogniaIDError] = useState<boolean>(false);
  ...
  const debouncedAddr = useDebounce(polygonAddr, 500);
  ...
  useEffect(() => {
    ...
    if (theirID.startsWith('https://incognia/#')) {
      setTheirID(theirID.replace('https://incognia/#', ''));
    } else {
      generateNewEphKey();
    }
  ...
  return (
    <div style={{ paddingTop: '1rem' }}>
      <p>
        ...
        controlled by the owner of Incognia ID.
      </p>
      <form
        className="lane"
        onSubmit={() => {
      ...
      >
        <div className="input-container">
          <input
            ...
            spellCheck="false"
            autoComplete="off"
            placeholder="Enter receiver Incognia ID"
            onChange={handleIDInput}
          />
          <label htmlFor="incogniaID">INCOGNIA ID</label>
        </div>
      </form>
      ...
      {isConnected && balance && (
        <>
          <div>
            ...
            >
              ...
              <button
                className="hbutton"
                color="success"
                disabled={!write || isLoading || amountError || incogniaIDError}
                onClick={(e) => {
                  e.preventDefault();
                  write?.();
                }}
          ...
          {incogniaIDError && (
            <div className="lane">
              <p className="message error">Invalid Incognia ID</p>
            </div>
          )}
          {isSuccess && !isError && !isPrepareError && (
            <div className="lane">
              <p className="message">
                ...
                <a
                  href={`https://blockscout.com/gnosis/chiado/tx/${data?.hash}`}
                  className="link-text"
                  target="_blank"
                  rel="noreferrer"
                >
                  View on Gnosis Chiado Testnet{' '}
                  <FontAwesomeIcon
                    icon={faArrowRight}
                    transform={{ rotate: -45 }}
                  />
